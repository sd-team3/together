...
//주소를 사용하지 않고 현재 위치 기준 지도 띄울 때 쓰는 함수
async function loadMapByLocation() {
    try {
        const { lat, lng } = await getXY();
...
async function loadMapByAddress() {
    try {
        const lat = 37.5600;
        const lng = 126.8000;
        const mapElement = document.getElementById('map');
...
async function loadMarker(crews) {
    const markers = null;
    try {
        kakao.maps.load(() => {
