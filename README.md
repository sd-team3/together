...
같은 스포츠를 즐기는 사람들끼리 정기모임·번개모임을 만들고 참가할 수 있는 웹 서비스.
모임 참가부터 실시간 채팅, GPS 출석 체크, 매너 점수 관리까지 하나의 플랫폼에서 제공.
...
서버 실행 후 브라우저와 시크릿 모드를 동시에 열어 서로 다른 계정으로 로그인하여 테스트

1. 회원가입 → 로그인
2. 정기모임 생성 → 크루원 계정으로 가입 신청 → 모임장 계정에서 승인/거절 → 채팅방 자동 입장 확인
3. 번개모임 생성 (날짜/시간/장소 설정, 카카오 지도 기반 위치 지정) → 크루원 계정으로 참가 신청 → 승인/거절 흐름 확인
4. 모임 채팅방 입장 → 실시간 메시지 송수신 확인
5. 친구 요청 → 수락 / 거절 실시간 처리 확인
6. 모임 시작 시 알림 실시간 수신 확인
7. GPS 출석 체크 → 모임 장소 500m 이내 접근 후 출석 처리 → 매너 점수 자동 부여 / 차감 확인
8. 스케줄러 동작 → 모임 상태 자동 변경 (모집 → 활동 → 종료) 확인
9. 설정 → 프로필 공개 범위 변경 (전체 공개 / 비공개) → 활동이력·매너점수 연동 확인
10. 설정 → 다크모드 토글 → 화면 전환 확인
